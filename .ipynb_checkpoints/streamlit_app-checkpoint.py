...
# Feature Analysis Page
    ...
    key_features = ['CreditScore', 'Age', 'Tenure', 'Balance', 'NumOfProducts', 
                    'IsActiveMember', 'EstimatedSalary', 'Exited']
    corr_matrix = df[key_features].corr()
    corr_with_churn = corr_matrix['Exited'].drop('Exited').sort_values(ascending=False)
    
    fig = px.bar(x=corr_with_churn.values, y=corr_with_churn.index,
                 orientation='h', title="Correlation with Churn",
                 color=corr_with_churn.values, color_continuous_scale='RdBu')
    ...
    col1, col2 = st.columns(2)
    with col1:
        fig = px.histogram(df, x='Age', color='Exited', title="Age Distribution",
                          color_discrete_map={0: 'blue', 1: 'red'})
        st.plotly_chart(fig, use_container_width=True)
    with col2:
        fig = px.histogram(df, x='Balance', color='Exited', title="Balance Distribution",
                          color_discrete_map={0: 'blue', 1: 'red'})
        st.plotly_chart(fig, use_container_width=True)
